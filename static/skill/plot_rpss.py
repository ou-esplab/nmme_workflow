#!/usr/bin/env python3
"""
Plot RPSS skill maps for NMME hindcasts.
...
Usage example:
    conda run -n nmme_workflow_env python static/skill/plot_rpss.py \
        --var prec --init-month 1 --lead 1
...
def _to_crs180(da: xr.DataArray) -> tuple:
    ...
    # Normalise to 0-360 if needed
    if float(lon_geo.min()) < 0:
        lon_geo = (lon_geo + 360) % 360
    lon_crs = lon_geo - 180          # CRS coords: -180 to 179
...
def _load_model(skilldir: Path, model: str, var: str) -> xr.DataArray | None:
    pattern = list(skilldir.glob(f"{model}.{var}.rpss.*.nc"))
    if not pattern:
        return None
...
def _plot_panel(ax, da: xr.DataArray, title: str, lat_extent: tuple, var: str = "") -> object:
    """Draw one RPSS panel on ax; return the mappable."""
...
def main() -> int:
    ...
    if args.models == "ALL":
        ...
        available = [
            ...
            for p in skilldir.glob(f"*.{args.var}.rpss.*.nc")
        ]
    ...
    if not models:
        print(f"[ERROR] No skill files found in {skilldir} for var={args.var}")
        return 1
