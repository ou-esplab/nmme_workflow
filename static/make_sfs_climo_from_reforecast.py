...
def main() -> int:
    ...
    for f in files:
        if args.verbose:
            print(f"  {f}")
        # Open with chunks={} so Dask manages memory — avoids allocating the
        # full (n_inits × leads × members × lat × lon) array at once.
        ...
        try:
            ds_i = xr.open_dataset(f, decode_times=True, chunks={})
        except Exception:
